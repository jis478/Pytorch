...
class Discriminator_Y(nn.Module):
    def __init__(self, size, norm='in', activation='relu', pad_type='zero'):
        ...
        self.final_linear = nn.Sequential(
            LinearBlock(dims[4] * 2 * 2, dims[4], norm='none', activation=activation),
            LinearBlock(dims[4], 1, norm='none', activation='none')
        )
...
class Discriminator_U(nn.Module):
    def __init__(self, size, norm='in', activation='relu', pad_type='zero'):
        ...
        for i in range(log_size, 4, -1):
            out_dim = dims[2 ** (i - 1)]
            blocks.append(ResBlock(in_dim, out_dim, norm='in', activation=activation, pad_type=pad_type, downsample=True))
            in_dim = out_dim
...
class ResBlocks(nn.Module):
    def __init__(self, num_blocks, in_dim, out_dim, norm='in', activation='relu', pad_type='zero'):
        super(ResBlocks, self).__init__()
        self.model = []
        for i in range(num_blocks):
            self.model += [ResBlock(in_dim, out_dim, norm=norm, activation=activation, pad_type=pad_type)]
        self.model = nn.Sequential(*self.model)
...
class ContentEncoder(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type):
        ...
        for i in range(n_downsample):
            ...
            dim *= 2
        # residual blocks
        self.model += [ResBlocks(n_res, dim, dim, norm=norm, activation=activ, pad_type=pad_type)]
        self.model = nn.Sequential(*self.model)
        self.output_dim = dim
